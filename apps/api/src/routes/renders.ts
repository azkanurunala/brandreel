...
// rendersRouter.use(requireAuth) tidak aman (semua router di-mount di root).
...
// POST /campaigns/:id/renders — antre render baru (cuma milik akun sendiri)
rendersRouter.post("/campaigns/:id/renders", requireAuth, async (req, res) => {
...
// GET /campaigns/:id/renders — daftar render campaign (cuma milik akun sendiri)
rendersRouter.get("/campaigns/:id/renders", requireAuth, async (req, res) => {
...
rendersRouter.get("/renders/:id", requireAuth, async (req, res) => {
  const item = await prisma.render.findUnique({ where: { id: req.params.id }, include: { campaign: { select: { accountId: true } } } });
  if (!item || item.campaign.accountId !== req.accountId) return res.status(404).json({ error: "Render tidak ditemukan" });
  res.json(item);
});
